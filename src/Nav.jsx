...
const Nav = () => {
  ...
  return (
    <nav className="fixed top-0 z-50 w-full h-[65px] bg-blue-950 shadow-md shadow-gray-600 flex justify-between items-center px-6 md:px-20">
      ...
      {/* Desktop Menu */}
      <ul className="hidden md:flex text-white font-sans font-medium">
        {menuItems.map((item) => (
          <li
            key={item.to}
            className="ml-10 text-sm uppercase cursor-pointer hover:text-gray-300 transition-colors"
          >
      ...
      >
        ...
        {menuItems.map((item) => (
          <Link
            ...
            duration={600}
            onClick={handleClick}
            className="py-6 text-4xl uppercase text-white font-sans hover:text-gray-300 transition-colors cursor-pointer"
          >
            {item.name}
          </Link>
        ))}
